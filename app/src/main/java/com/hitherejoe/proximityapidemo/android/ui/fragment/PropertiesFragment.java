...
public class PropertiesFragment extends Fragment {
    ...
    public static PropertiesFragment newInstance(Mode mode) {
        PropertiesFragment propertiesFragment = new PropertiesFragment();
        Bundle args = new Bundle();
        args.putSerializable(EXTRA_MODE, mode);
        propertiesFragment.setArguments(args);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (mPropertiesMode == mPropertiesMode.UPDATE && mBeacon == null) throw new IllegalArgumentException("Properties fragment requires a beacon!");
    ...
    private void registerBeacon() {
        ...
        if (mPropertiesMode == Mode.UPDATE) {
            hasStatusChanged = beacon.status != mBeacon.status;
        }
        ...
        if (beaconStabilitySpinner.getSelectedItemPosition() > 0) {
            Beacon.Stability stability = Beacon.Stability.fromString(beaconStabilitySpinner.getSelectedItem().toString());
            beacon.expectedStability = stability;
        }
        ...
        mSubscriptions.add(AppObservable.bindFragment(this,
                ...
                .subscribe(new Subscriber<Beacon>() {
                    ...
                    public void onError(Throwable error) {
                        Log.d("ERROR", "There was an error : " + error.getMessage());
                        if (error instanceof RetrofitError) {
                            DialogFactory.createRetrofitErrorDialog(getActivity(), (RetrofitError) error);
                        } else {
                            DialogFactory.createSimpleOkErrorDialog(getActivity(), getString(R.string.dialog_error_title), getString(R.string.dialog_general_error_Message)).show();
                        }
                    }

                    @Override
                    public void onNext(Beacon beacon) {
                        //set activity result to beacon
                        if (mPropertiesMode == Mode.UPDATE) {
    ...
    private void populateBeaconForm() {
        ...
        mPlaceIdEditText.setText(mBeacon.placeId != null ? mBeacon.placeId : " ");
        mDescriptionEditText.setText(mBeacon.description != null ? mBeacon.description : " ");
        LatLng latLng = mBeacon.latLng;
        mLatitudeEditText.setText(latLng.latitude != null ? String.valueOf(mBeacon.latLng.latitude) : " ");
        mLongitudeEditText.setText(latLng.longitude != null ? String.valueOf(mBeacon.latLng.longitude) : " ");
        if (mBeacon.status != null) {
    ...
    private void setupViewBeaconForm() {
        mBeaconNameEditText.setText(mBeacon.beaconName);
        mAdvertisedIdEditText.setText(mBeacon.advertisedId.id);
